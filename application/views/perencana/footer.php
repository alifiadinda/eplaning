	...
	function hapusRincian(id_rincian) {
		...
		var r = confirm("Apakah anda yakin ingin menghapus usulan ini?");
		  if (r == true) {
			$.ajax({
				type  : 'POST',
				url   : '<?php echo base_url()?>index.php/C_Perencana/hapusRincian/'+id_rincian,
				async : false,
				dataType : 'json',
...
$(document).ready(function() {
	...
	/*TAMBAH USULAN*/
		$('#tambahUsulan').submit(function(e){
			...
			$.ajax({
				type : 'POST',
				url  : '<?php echo site_url(); ?>/C_Perencana/tambahRincian',
				dataType : 'JSON',
				data : {
					id_usulan:id_usulan,
	...
	/*EDIT JUMLAH USULAN*/
		$('#editJumlah').submit(function(e){
			...
			$.ajax({
				type : 'POST',
				url  : '<?php echo site_url(); ?>/C_Perencana/updateJumlah',
				dataType : 'JSON',
				data : {
